...
#include "processors/RISC-V/rv_fpu.h"
...
class RVSSFP : public RipesVSRTLProcessor {
...
public:
  RVSSFP(const QStringList &extensions)
      : RipesVSRTLProcessor("Single Cycle RISC-V Processor FP") {
  ...
  SUBCOMPONENT(fpRegisterFile, TYPE(RegisterFile<FLEN, false>));
  SUBCOMPONENT(alu, TYPE(ALU<XLEN>));
  SUBCOMPONENT(fpu, TYPE(FPU<FLEN>));
